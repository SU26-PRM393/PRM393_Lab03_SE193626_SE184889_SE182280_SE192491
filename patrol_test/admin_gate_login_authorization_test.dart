...
void main() {
  patrolTest(
    'admin gate login authorizes right credentials',
    ($) async {
      ...
      await _waitForAdminOnlyNavigation($);
    },
  );
}
...
Future<void> _waitForAdminOnlyNavigation(PatrolIntegrationTester $) async {
  final userManagementLabel = find.text('Người dùng');

  if (userManagementLabel.evaluate().isNotEmpty) {
    return;
  }
  ...
  if (menuButton.evaluate().isNotEmpty) {
    ...
    await _waitForCondition(
      $.tester,
      () => userManagementLabel.evaluate().isNotEmpty,
      timeout: const Duration(seconds: 15),
    );
    return;
  }

  fail('Unable to locate admin-only navigation item after login.');
}
